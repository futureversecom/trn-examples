...
export async function main() {
	...
	const tx = await erc1155Precompile
		.connect(wallet)
		.burnBatch(wallet.address, [serialNumber1, serialNumber2], [burnAmount, burnAmount]);
	const receipt = await tx.wait();

	const event = (receipt?.events as any)[0].event;
	const { operator, from, to, ids, balances } = (receipt?.events as any)[0].args;

	assert(event === `TransferBatch`, `Incorrect event ${event}`);
	assert(from.toString() === wallet.address, `Incorrect from ${from.toString()}`);
	assert(operator.toString() === wallet.address, `Incorrect operator ${operator.toString()}`);
	assert(
		to.toString() === "0x0000000000000000000000000000000000000000",
		`Incorrect to ${to.toString()}`
	);
	assert(ids[0].toNumber() === serialNumber1, `Incorrect id0 ${ids[0].toString()}`);
	assert(ids[1].toNumber() === serialNumber2, `Incorrect id1 ${ids[1].toString()}`);
	assert(
