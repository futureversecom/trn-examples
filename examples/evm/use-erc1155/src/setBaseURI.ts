...
export async function main() {
	...
	const serialNumberNew = await createToken(erc1155Precompile, initialIssuance, wallet);
	...
	assert(baseURI.toString() === newMetadataPath, `Incorrectly set base uri ${baseURI.toString()}`);

	const checkURI = await erc1155Precompile.uri(serialNumberNew);
	// Verify URI set correctly
	assert(
		checkURI.toString() === `https://example.com/sft/updated/${serialNumberNew}`,
