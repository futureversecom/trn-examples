...
export async function main() {
	...
	const bal1 = await erc1155Precompile.balanceOf(receiverAddress, serialNumber1);
	const bal2 = await erc1155Precompile.balanceOf(receiverAddress, serialNumber2);
	assert(
	...
	const { from, to, operator, ids, balances } = (receipt?.events as any)[0].args;
	...
	assert(operator === wallet.address, `Incorrect operator ${operator.toString()}`);
	assert(ids[0].toString() === serialNumber1.toString(), `Incorrect ids field ${ids[0]}`);
	assert(ids[1].toString() === serialNumber2.toString(), `Incorrect ids field ${ids[1]}`);
	assert(
		balances[0].toString() === initialIssuance1.toString(),
		`Incorrect balances field ${balances[0]} expected ${initialIssuance1.toString()}`
	);
	assert(
		balances[1].toString() === initialIssuance2.toString(),
		`Incorrect balances field ${balances[1]} expected ${initialIssuance2.toString()} `
	);
}
