...
export async function main() {
	...
	const tx = await erc20Precompile.connect(wallet).approve(target, amountToApprove);
	...
	assert(owner === "0xE04CC55ebEE1cBCE552f250e85c57B70B2E2625b", `Incorrect from field ${owner}`);
	assert(spender === "0x25451A4de12dcCc2D166922fA938E900fCc4ED24", `Incorrect to field ${spender}`);
	assert(value.toString() === amountToApprove.toString(), `Incorrect to amount ${value}`);
}
