...
import { convertStringToHex, Transaction, Wallet, xrpToDrops } from "xrpl";
...
export async function main() {
  ...
  api.query.system.events((events: any[]) => {
    ...
    events.forEach((record) => {
      ...
      if (
        event.section === "xrplBridge" &&
        event.method === "TransactionAdded"
      ) {
        console.log(
          `XRPL Bridge transaction added successfully, for event id ${event.data[0]}`
        );
      }

      if (event.section === "xrplBridge" && event.method === "ProcessingOk") {
        console.log(
          `XRPL Bridge transaction executed successfully, for event id ${event.data[0]}`
        );
