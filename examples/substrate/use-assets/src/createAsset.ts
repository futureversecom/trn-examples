...
export async function main() {
	...
	const { result: createExtrinsicResult } = await sendExtrinsic(createExtrinsic, caller, {
		log: console,
	});
	const [createExtrinsicEvent] = filterExtrinsicEvents(createExtrinsicResult.events, [
		"Assets.Created",
	]);
	...
	const { result: setMetadataResult } = await sendExtrinsic(setMetadataExtrinsic, caller, {
		log: console,
	});
	const [setMetadataEvent] = filterExtrinsicEvents(setMetadataResult.events, ["Assets.Created"]);
